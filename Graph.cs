...
public class Graph<V, E> where E : class
{
    ...
    public override string ToString()
    {
        ...
        foreach (var vertex in vertices)
        {
            ...
            string vertexString = $"{index} V({vertex})";

            var edges = GetVertexEdges(vertex);

            foreach (var edge in edges)
            {
                int edgeVertexIndex = edge.GetVertexIndex();

                var edgeVertex = GetVertex(edgeVertexIndex);
                string edgeString = $"\n{vertex}--{edgeVertex}";

                vertexString += edgeString;
            }

            graphString += vertexString + "\n";
        }
